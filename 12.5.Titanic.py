...
def load_data(file_name, is_train):
    ...
    pd.set_option('display.width', 200)  #设置显示每一行的宽度，防止回车显示，数据不好看
    print ('data.describe() = \n', data.describe()) #此处只统计数值型的数据，统计每个数值的意义

    # 性别的处理
    data['Sex'] = data['Sex'].map({'female': 0, 'male': 1}).astype(int)
    #由于性别已经转化为数值型数据，所以统计出来了
    print ('data.describe() = \n', data.describe())
    ...
    data = pd.concat([data, embarked_data], axis=1)  #给数据做一个 纵向连接
    print (data.describe())
